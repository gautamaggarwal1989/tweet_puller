...
class ExportTweets:
	...
	def __init__(self, username, logging):
		...
		self.auth = tweepy.OAuthHandler(self.CUSTOMER_KEY, self.CUSTOMER_SECRET)
	...
	def get_user_tweets(self):
		...
		apis. ''' 
		tweets = []
		# Write the code here to handle the case where users are not present anymore
		if not self.user_exists_on_twitter(self.user_name):
			return None
		for page in self.handle_limit_issue(tweepy.Cursor(
				self.api.user_timeline, screen_name=self.user_name, count=self.TWEET_PER_PAGE).pages()):
			for tweet in page:
				tweet_info = {}
				tweet_info['tweet_id'] = tweet.id_str
				tweet_info['tweet_text'] = tweet.text
				tweet_info['created_at'] = tweet.created_at.strftime(self.DATE_FORMAT)
	...
	def handle_limit_issue(self, cursor):
		...
		error is encountered, handles it with sleeping for 15 minutes.'''
		while True:
			try:
			    yield cursor.next()
			except tweepy.TweepError:
				self.logging.error('Hit exceeded allowed hits... going to sleep for a few')
			    time.sleep(900)
